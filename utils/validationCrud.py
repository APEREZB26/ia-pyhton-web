...
def employee_validation_edit(mysql, phone):
    ...
    if regphone is None:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT phone FROM proyect.employee WHERE phone={}".format(phone)
        )
...
def employee_validation_add(mysql, fullname, email, dni, password, phone):
    ...
    if regfullname is None:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT fullname FROM proyect.employee WHERE fullname='{}'".format(fullname)
        )
        ...
        if fullnameBD == None:
            if regemail is None:
                ...
                cursor = conn.cursor()
                cursor.execute(
                    "SELECT email FROM proyect.employee WHERE email='{}';".format(email)
                )
                ...
                if emailBD == None:
                    if regdni is None:
                        ...
                        cursor = conn.cursor()
                        cursor.execute(
                            "SELECT dni FROM proyect.employee WHERE dni={};".format(dni)
                        )
                        ...
                        if dniBD == None:
                            if regphone is None:
                                ...
                                cursor = conn.cursor()
                                cursor.execute(
                                    "SELECT phone FROM proyect.employee WHERE phone={}".format(
                                        phone
                                    )
                                )
                                phoneBD = cursor.fetchone()
